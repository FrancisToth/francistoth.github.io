...
```
While being able to store heterogeneous elements, a typical `List` does not keep track of their types. Instead, it falls back on the first common type in the type hierarchy (`Any` in this case):
```scala
val xs = List(
...
```
This approach solves the problem but remains very verbose. We can improve this by adding an operator designed to prepend an element to an `HList`:
```scala
object HList {
  // ...
...
```scala
  ...
  43 *: (new Foo(42))
```
This trick simplifies the composition of an `HList` at the value-level but not at the type-level. The type of `xs` is still very verbose. So let's try to do the same improvement at the type-level using some type aliases:
```scala
object HList {
...
val xs: String :*: Boolean :*: Int :*: Empty =
    "a string" :*: true :*: 42 :*: Empty
```
Notice that infix operators work at the value-level but also at the type-level. Using this technique, we made the type of an `HList` very readable. What about extracting elements from an `HList` now? To achieve this, we could use an [Extractor](https://docs.scala-lang.org/tour/extractor-objects.html):
```scala
object :*: {
...
val s :*: b :*: i :*: _ = 
  "a string" :*: true :*: 42 :*: Empty
```
So far, we focused mainly on adding one element to an `HList` but what about concatenating two `HList`s? Before getting deeper, let's look at the code so far:
```scala
sealed trait HList
object HList {
...
```
If you look closely, you will see that this design is flawed. To convince ourselves, let's look at what this would imply for `Empty`:
```scala
object HList {
  // ...
  case object Empty extends HList {
...
```
`Append[That <: HList]` is, therefore, a type-level function taking and returning a `That`, while `++` is a value-level function taking a value typed as a `That` and returning (once expanded) a `That`. In other words, there is a symmetry between `Append[That <: HList]` and `++`, each living respectfully in the type-level world and the value-level world. We can now fix the definition of `Append` in `HList`:
```scala
sealed trait HList {
...
```
Once again, notice the symmetry between `tail.Append[That]` and `tail ++ that`. To give you a full picture, here's the final code: 
```scala
sealed trait HList {
...
val s :*: i :*: _ = x ++ y
```
The biggest takeaways of this exercise are first that we can get the same flexibility at the value and the type level, and secondly that thinking about types as functions can be a breakthrough when you tackle Type programming. Possibilities are not endless as there are cases when the compiler won't be able to track types properly (especially when you start introducing [GADTs](/2019/11/30/gadt.html)) but are sufficient enough to provide correctness at the type level in most of the cases.
